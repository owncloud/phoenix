...
export default {
  ...
  methods: {
    ...
    $_loader_getDavFilePath(mediaFile, query = null) {
      ...
      // download the file.
      const [path, signedQuery] = mediaFile.downloadURL.split('?')
      const fileUrl = this.configuration.server + path.substring(1)
      ...
      return [fileUrl, combinedQuery].filter(Boolean).join('?')
    },
